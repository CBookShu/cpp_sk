...
struct logger : public module_base_t {
  ...
  static int 
      cb(struct context_t * context, int type, int session, uint32_t source, std::any& a, size_t sz) {
    ...
    if (type == PTYPE_TEXT) {
        ...
        auto timefmt = std::format("{}", std::chrono::zoned_time{zone, current_point});
        auto output = std::format("[{}][{:08x}]{}\n",
            std::chrono::zoned_time{zone, current_point},
            source,*s);
        if (log->fp) {
