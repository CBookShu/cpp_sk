...
auto &server::get_slot(int id) { return slots[id % slots.size()]; }

int server::socket_listen(context_t *ctx, const char *host, int port,
                          int backlog) {
  try {
...
int server::socket_connect(context_t *ctx, const char *host, int port) {
  try {
    ...
    asio::co_spawn(
        poll,
        [this, &slot, id = id,
         query = std::move(query)]() -> asio::awaitable<void> {
          auto &slot = get_slot(id);
          ...
          try {
            ...
            auto ep = co_await asio::async_connect(*slot.tcp(), it,
                                                   asio::use_awaitable);
...
void server::socket_start(context_t *ctx, int id) {
  poll.post([this, handle = ctx->handle, id = id]() {
    ...
    asio::co_spawn(
        poll,
        [this, id = id, handle = handle]() -> asio::awaitable<void> {
...
int cpp_sk::server::socket_send(context_t *ctx, int id, std::string buf) {
  ...
  asio::co_spawn(
      poll,
      [this, id = id, handle = ctx->handle, offset,
       buf = std::move(buf)]() -> asio::awaitable<void> {
        auto &slot = get_slot(id);
        if (!slot.check_can_write(id)) {
          co_return;
        }
          ...
          co_await std::visit(
              overload(
                  [&](socket_t::tcp_ns::socket &s) -> asio::awaitable<void> {
                    while (!slot.wlist.empty()) {
                      auto [ec, sz] = co_await asio::async_write(
                          s, asio::buffer(slot.wlist.front()),
                  ...
                  [&](socket_t::udp_ns::socket &s) -> asio::awaitable<void> {
                    while (!slot.wlist.empty()) {
                      auto [ec, sz] = co_await s.async_send(
        ...
        // write fail
        if (slot.type == socket_type::halfclose_read || slot.close) {
          slot.clear();
...
void cpp_sk::server::close_socket(context_t *ctx, int id) {
  poll.post([this, id]() {
    auto &slot = get_slot(id);
    if (slot.id != id || slot.type == socket_type::invalid || slot.close) {
      return;
    }
  ...
  co_return co_await std::visit(
      overload(
          [&](tcp::acceptor &a) -> asio::awaitable<void> {
            ...
            try {
              for (;;) {
                if (slot.id != id || slot.type != socket_type::listen) {
                  result.data.const_assgin("invalid socket");
                  forward_message(skynet_socket_type::error, result);
                  co_return;
                }
                ...
                auto sock =
                    co_await slot.acceptor()->async_accept(asio::use_awaitable);
          ...
          [&](tcp::socket &s) -> asio::awaitable<void> {
            ...
            try {
              ...
              for (;;) {
                if (slot.id != id ||
                    (slot.type != socket_type::connected &&
                     slot.type != socket_type::halfclose_write)) {
                  break;
                }

                result.opaque = slot.opaque;

                auto sz = co_await s.async_read_some(asio::buffer(buff),
                                                     asio::use_awaitable);
...
bool cpp_sk::socket_t::check_pstart(int id_) {
  ...
  if (type == socket_type::paccept || type == socket_type::plisten ||
...
bool cpp_sk::socket_t::check_can_start(int id_) {
  ...
  */
  ...
  if (type == socket_type::connected || type == socket_type::halfclose_write ||
...
bool cpp_sk::socket_t::check_can_write(int id_) {
  ...
  */
  ...
  if (type == socket_type::connected || type == socket_type::halfclose_read) {
    return true;
  }
